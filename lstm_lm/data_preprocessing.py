...
def load_data(filename="data/lstm_test.csv", vocabulary_size=2000, min_sent_characters=0):
    ...
    with open(filename, 'r', encoding='utf-8') as f:   # add  "encoding='utf-8'" to solve the problem
...
def train_with_sgd(model, X_train, y_train, learning_rate=0.001, nepoch=20, decay=0.9,
    ...
    for epoch in range(nepoch):
        ...
        for i in range(len(y_train)):
            ...
            loss = model.calculate_loss(X_train[:100], y_train[:100])         
    return model
...
#def load_model_parameters_theano(path, modelClass=LSTMRNN):
